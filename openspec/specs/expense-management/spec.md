...
## Purpose
Enable authenticated users to create, read, update, and delete personal expenses with support for categorization, payment method tracking, and filtering.
...
The system SHALL allow a user to update their own expenses. Admin SHALL update any expense. Fields: description, amount, date, categoryId, paymentMethodId, status. Only Admin can change status.
